...
set.seed(520)
train_index <- sort(sample(1:nrow(train_full), 5*nrow(train_full)/10))
...
test <- train_full[test_index, ]

write.csv(train, file = 'subset1.csv', row.names = F)
write.csv(test, file = 'subset2.csv', row.names = F)
