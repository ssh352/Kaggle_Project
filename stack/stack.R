...
predmat <- exp(predict(stack_gbm, newdata = test[, 2:7], n.trees = n.trees))
dim(predmat)
...
pred_s <- exp(predict(stack_gbm_full, newdata = df_s, n.trees = 9900))

submission <- read.csv("Prediction/sample_submission.csv")
submission$loss <- as.numeric(pred_s)
write.csv(submission, file = 'submissionNov25_gbmstack5.csv', row.names = F)
...
submission <- read.csv("Prediction/sample_submission.csv")
submission$loss <- as.numeric(pred_s)
write.csv(submission, file = 'submissionNov25_xgbstack2.csv', row.names = F)
